...
`include        "Sdram_Params.h"
  ...
  reg [9:0]       rd_cnt;          //一次突发读操作过程时间计数器  
  reg [9:0]       wr_cnt;          //一次突发写操作过程时间计数器
  ...
  localparam rd_ACT_TIME  = 1'b1,
             ...
             rd_BUST_TER  = SC_RCD+SC_BL+SC_CL-(SC_CL-1),
             rd_PRE_TIME  = SC_RCD+SC_BL+SC_CL+1-(SC_CL-1),
             rd_END_TIME  = SC_RCD+SC_CL+SC_BL+1;
  ...
  begin
    if(!Rst_n)
      Rd_data_vaild <= 1'b0;
    else if((rd_cnt > SC_RCD+SC_CL+1)
            &&(rd_cnt <= SC_RCD+SC_CL+SC_BL+1))
      Rd_data_vaild <= 1'b1;
    else
      Rd_data_vaild <= 1'b0;
  end
  ...
  //读数据
  assign Rd_data = Rd_data_vaild ? Dq:16'h0000;
