...
`include        "Sdram_Params.h"
  ...
  wire [9:0]          fifo_rduse;     //写FIFO模块可读数据量
  wire [9:0]          fifo_wruse;     //读FIFO模块已写数据量
  reg [23:0]          wr_sdram_addr;  //写SDRAM的地址
  reg [23:0]          rd_sdram_addr;  //读SDRAM的地址
  reg                 sd_wr_req;      //请求写数据到SDRAM
  reg                 sd_rd_req;      //请求向SDRAM读数据
...
//  assign sd_wr_req = (!Wr_load)&&(fifo_rduse >= SC_BL)?1'b1:1'b0;
  ...
  begin
    if(!Rst_n)
      sd_wr_req <= 1'b0;
    else if((!Wr_load)&&(fifo_rduse >= SC_BL))
      sd_wr_req <= 1'b1;
    else
      sd_wr_req <= 1'b0;
  end
...
//  assign sd_rd_req = (!Rd_load)&&(fifo_wruse[7]==1'b0)?1'b1:1'b0;
  ...
  begin
    if(!Rst_n)
      sd_rd_req <= 1'b0;
    else if((!Rd_load)&&(fifo_wruse < SC_BL))
      sd_rd_req <= 1'b1;
    else
      sd_rd_req <= 1'b0;
  end
